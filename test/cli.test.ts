...
import { mkdtempSync, mkdirSync, rmSync, writeFileSync, utimesSync, readdirSync } from 'node:fs'
...
describe('runCli', () => {
  ...
  it('clean without --yes lists targets but deletes nothing', () => {
    ...
    expect(code).toBe(0)
    expect(readdirSync(saveDir).length).toBe(2)
  ...
  it('clean --all --yes deletes all annotated files', () => {
    ...
    expect(code).toBe(0)
    expect(readdirSync(saveDir)).toEqual(['keep.txt'])
  })

  it('clean --keep 1 --yes keeps the newest only', () => {
    touch('annotated-a.png', 30); touch('annotated-b.png', 20); touch('annotated-c.png', 10)
    runCli(['clean', '--keep', '1', '--yes'], io())
    expect(readdirSync(saveDir)).toEqual(['annotated-c.png'])
  })
